...
def annotate_frame(frame, polygon_points=None, tracks=None, primary=None,debug=None,detections=None):
    ...
    for track in tracks or []:
        x1,y1,x2,y2=map(int,track.bbox); is_primary=str(track.track_id)==primary_id; color=(0,60,255) if is_primary else (0,220,255)
        anchor=(int((x1+x2)/2),y2); cv2.rectangle(output,(x1,y1),(x2,y2),color,max(2,w//1000)); cv2.circle(output,anchor,6,(255,0,255),-1)
        label=f"{'PRIMARY ' if is_primary else ''}{track.vehicle_class} #{track.track_id} {track.confidence:.2f} age={track.track_age} tsu={track.time_since_update} overlap={track.overlap:.2f} inside={track.anchor_inside}"
