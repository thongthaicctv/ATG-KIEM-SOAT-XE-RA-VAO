...
class CameraCard(QGroupBox):
    ...
    def update_data(self,state,track="-",session="-",vehicle="-",fps="-",raw=0,detected=0,inside=0,inference="-",tracker="IDLE",configured_preview_fps=None,candidates=None,parked=None,leaving=None,recovery_pending=None,open_db=None,session_runtime_mismatch=False):
        if configured_preview_fps is not None: self.preview_fps=float(configured_preview_fps)
        self.set_state(state); self.details.setText(f"Track hiện tại: {track}\nSession: {session}\nLoại xe/sức chứa: {vehicle}\nFPS AI: {fps}\nPreview: {self.preview_fps:g} FPS"); warning="FRAME_DELAY | " if tracker=="FRAME_DELAY" else ""; runtime_line=f"\nCandidate: {candidates} | Đang đỗ: {parked} | Đang rời: {leaving}\nRecovery: {recovery_pending} | Open DB: {open_db}" if candidates is not None else ""; mismatch="\nSESSION_RUNTIME_MISMATCH" if session_runtime_mismatch else ""; self.debug.setText(f"{warning}Raw: {raw} | Vehicle: {detected} | Trong polygon: {inside}\nInference: {inference} ms | Tracker: {tracker}{runtime_line}{mismatch}"); self.debug.setStyleSheet("color:#d50000;font-weight:800;font-family:Consolas" if tracker=="FRAME_DELAY" or session_runtime_mismatch else "color:#455a64;font-family:Consolas")
