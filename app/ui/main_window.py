...
class MainWindow(QMainWindow):
    def __init__(self,settings):
        ...
        self.pipeline_errors={}; self.last_capture_frame={}; self.last_ai_result={}; self.zones={}; self.engines={}; self.frames={}; self.raw_frames={}; self.last_payload={}; self.last_preview_telemetry={}; self.active={}; self.preview_dialogs={}; self.setWindowTitle("Parking Monitoring System - Phase 1.4"); self.resize(1280,800); self._build_ui(); self.reload(start_workers=False); self._recover(); self.reload(start_workers=True); self.pipeline_watchdog=QTimer(self); self.pipeline_watchdog.setInterval(2000); self.pipeline_watchdog.timeout.connect(self._check_pipeline_health); self.pipeline_watchdog.start()
    def _build_ui(self):
        ...
        tabs.addTab(self.camera_page,"Camera"); tabs.addTab(self.monitor,"Giám sát"); self.history_tab_index=tabs.addTab(self.history,"Lịch sử phiên đỗ"); tabs.currentChanged.connect(self._on_tab_changed); splitter.addWidget(tabs); self.event_log=EventLogWidget(); splitter.addWidget(self.event_log); splitter.setSizes([620,180]); self.setCentralWidget(splitter); self.setStatusBar(QStatusBar()); self.statusBar().showMessage(f"Profile: {self.settings.runtime_profile} | tối đa {self.settings.max_cameras} camera | AI: {self.detector.name}")
    ...
    def reload(self,start_workers=True):
        ...
        for r,c in enumerate(cameras):
            ...
            if start_workers and c.enabled and configured and self.detector.enabled and c.id not in self.manager.items: self.manager.start_camera(c)
    ...
    def on_zone_frame(self,camera_id,frame,payload):
        ...
        stats=payload.get("stats",{}); open_db=len(self.parking.find_open_sessions_for_position(camera.id,camera.parking_position_code)); active_runtime=len(zone.vehicles); unmatched_db=max(0,open_db-zone.parked_count); stats.update({"current_track_count":sum(c.time_since_update==0 for c in candidates),"candidate_runtime_count":zone.candidate_count,"parked_runtime_count":sum(v.state=="OCCUPIED" for v in zone.vehicles.values()),"leaving_runtime_count":sum(v.state=="LEAVING" for v in zone.vehicles.values()),"recovery_pending_runtime_count":sum(v.state=="RECOVERY_PENDING" for v in zone.vehicles.values()),"open_session_count_database":open_db,"unmatched_open_session_count":unmatched_db}); inference_ms=float(stats.get("inference_ms",0)); actual_fps=1000/inference_ms if inference_ms>0 else 0; visible=sorted(zone.vehicles.values(),key=lambda v:(v.session_id is None,v.first_seen_at or now))[:3]; summary=" | ".join(f"{v.stabilized_class}:{v.state}:{v.session_code or '-'}" for v in visible) or "-"
        ...
        self.monitor.update_camera(camera_id,state=zone.state,track=stats["current_track_count"],session=summary,vehicle=f"{zone.parked_count}/{zone.capacity}",fps=f"{actual_fps:.2f}",raw=stats.get("raw_detections",0),detected=stats.get("vehicle_detections",0),inside=len(candidates),inference=f"{inference_ms:.1f}",tracker=stats.get("tracker_status","IDLE"),configured_preview_fps=camera.preview_fps,candidates=stats["candidate_runtime_count"],parked=stats["parked_runtime_count"],leaving=stats["leaving_runtime_count"],recovery_pending=stats["recovery_pending_runtime_count"],open_db=open_db,session_runtime_mismatch=bool(unmatched_db))
        self.log.debug("Zone aggregate camera=%s state=%s occupied=%s capacity=%s candidates=%s",camera.camera_code,zone.state,zone.parked_count,zone.capacity,zone.candidate_count)
