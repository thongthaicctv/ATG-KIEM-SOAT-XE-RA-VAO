...
class CameraDialog(QDialog):
    def __init__(self,camera=None,parent=None):
        ...
        self.fps.setValue(settings.processing_fps); self.conf.setValue(.20); self.imgsz.setCurrentIndex(self.imgsz.findData(settings.detector_image_size)); self.overlap.setValue(.20); self.motorcycles.setChecked(True); self.roi.setChecked(True); self.debug_overlay.setChecked(settings.ai_debug_overlay); self.park.setValue(5); self.exit.setValue(2); self.grace.setValue(3); self.miss_grace.setValue(5); self.enabled.setChecked(True)
        if camera:
